...
import pbftSimulator.replica.*;

public class Simulator {
	
	public static final int RN = 5;  						//replicas节点的数量(rn)
	
	public static final int FN = 2;							//恶意节点的数量
	
	public static final int CN = 1;						//客户端数量
	
	public static final int INFLIGHT = 1; 					//最多同时处理多少请求
	...
	public static void main(String[] args) {
		...
		boolean[] byzts = {false, false, false, false, true};
		...
		while(!msgQue.isEmpty()) {
			...
			case Message.REQUEST:
				for(int i : getPrimaryNodeIds()){
					reps[i].msgProcess(msg);
				}
				break;
			default:
			...
			if(requestNums - getStableRequestNum(clis) < INFLIGHT && requestNums < REQNUM) {
				clis[rand.nextInt(CN)].sendRequest(msg.receiveTime);
				requestNums++;
			}
			inFlyMsgLen -= msg.len;
		...
		double transactionsNum = getStableRequestNum(clis);
		double howLong = (double)timestamp/1000;
		double tps = transactionsNum/howLong;
	...
	public static ArrayList<Integer> getPrimaryNodeIds() {
		ArrayList<Integer> primaryNodeIds = new ArrayList<>();
		primaryNodeIds.add(0);
		primaryNodeIds.add(4);
		return primaryNodeIds;
	}

	public static boolean IsPrimaryNodeId(Integer i) {
		return getPrimaryNodeIds().contains(i);
	}
