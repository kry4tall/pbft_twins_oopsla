...
public class Client {
	...
	public void receiveCliTimeOut(Message msg) {
		...
		for(int i = 0; i < Simulator.RN; i++) {
			Message requestMsg = new RequestMsg("Message", t, id, id, i, cliTimeOutMsg.receiveTime + netDlys[i]);
			Simulator.sendMsg(requestMsg, sendTag);
		}
		//发送一条Timeout消息，以便将来检查是否发生超时
		setTimer(t, cliTimeOutMsg.receiveTime);
	}
